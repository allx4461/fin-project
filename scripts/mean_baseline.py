...
def train_model(df: pd.DataFrame):
    ...
    test_pred = np.full(len(x_test),np.mean(train_df['target_return']))
...
def run_grid_search(df: pd.DataFrame) -> pd.DataFrame:
    results = []
    val_metrics,test_metrics,strategy_metrics=validate(df,train_model(df)[0],train_model(df)[1])
    results.append({
...
def start():
    ...
    output_path = RESULTS_DIR / "metrics_mean_baseline.csv"
